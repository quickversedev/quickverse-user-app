...
const OrderDetailsSkeleton = ({ getColor }: { getColor: (key: ColorKey) => string }) => {
...
const OrderDetailsScreen = () => {
  ...
  // selectedOrder from the API may embed `review` / `complaint` (see raw
  // order JSON) even though the shared Order type doesn't declare them yet —
  // this widened alias lets us read them safely without an `any` cast at
  // every call site. See types/orderFeedback.ts for the TODO on Order itself.
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: getColor('background') }]}>
      <View style={[styles.container, { backgroundColor: getColor('background') }]}>
        ...
        >
          ...
          {/* Shop Details Section — not for a Daily Essentials order, whose kiranas the customer
                never sees. */}
          {!essentialsOrderId && (
            <>
              ...
              >
                {vendorDetails ? (
                  <View style={styles.shopDetailsContainer}>
                    <ThemeText style={[styles.shopName, { color: getColor('text') }]}>
                      {vendorDetails.name}
                    </ThemeText>
                    ...
                    {Boolean(vendorDetails.phone) && (
                      <View style={styles.contactRow}>
                        ...
                        >
                          ...
                          <ThemeText style={styles.callButtonText}>Call Shop</ThemeText>
                        </TouchableOpacity>
                      </View>
                    )}
          ...
          >
            {derivedItems.length === 0 ? (
              ...
              <View style={styles.itemsContainer}>
                ...
                {hasMoreItems && (
                  <TouchableOpacity
                    style={[styles.showMoreButton, { borderTopColor: getColor('border') }]}
                    onPress={() => setShowAllItems(!showAllItems)}
                    activeOpacity={0.7}
                  >
                    <ThemeText style={[styles.showMoreText, { color: getColor('primary') }]}>
          ...
          {/* OrderReviewCard reads order.review itself (embedded on the API
                payload) and renders the read-only "already reviewed" state
          ...
          {/* HelpCard reads order.complaint itself and, if present, shows a
                "Query already submitted" state + read-only modal instead of
                letting the user raise a second complaint. */}
          {!essentialsOrderId && (
            <HelpCard
